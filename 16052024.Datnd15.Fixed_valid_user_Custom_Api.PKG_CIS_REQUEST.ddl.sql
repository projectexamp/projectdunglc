CREATE OR REPLACE PACKAGE CIS_OPS_NCB."PCK_CIS_REQUEST" AS
  ...
  /*DATND15 NCB_CIC_PCB_2023_PM-232 START*/
  FUNCTION CHECK_LIMIT_ENQUERY(
			user_name IN VARCHAR2
) RETURN NUMBER;
  /*DATND15 NCB_CIC_PCB_2023_PM-232 END*/
/*Coding custom API Start*/
PROCEDURE IS_VALID_USER_EXITS(p_user   IN VARCHAR2,
							  P_OUT    OUT SYS_REFCURSOR);
/*Coding custom API END*/
END PCK_CIS_REQUEST;
...
CREATE OR REPLACE PACKAGE BODY CIS_OPS_NCB."PCK_CIS_REQUEST" AS
  ...
  PROCEDURE PR_CREATE_CIS_REQUEST (
    ...
    AS
       ...
       --Datnd15 NCB_CIC_PCB_2023_PM-232 START
       V_NUMBER_RECORD_TODAY NUMBER; 
      --Datnd15 NCB_CIC_PCB_2023_PM-232 END
    BEGIN
        V_ID_CIS_REQUEST:= SEQ_CIS_REQUEST.NEXTVAL;
       /*Datnd15 NCB_CIC_PCB_2023_PM-232  Start*/
       V_NUMBER_RECORD_TODAY := PCK_CIS_REQUEST.CHECK_LIMIT_ENQUERY(p_user);
      /*Datnd15 NCB_CIC_PCB_2023_PM-232 END*/
...
WHERE
	B.USER_NAME = nvl(p_maker, p_user);
		...
		--DATND15 NCB_CIC_PCB_2023_PM-232 START
       IF p_product_code IN ('R21','R11A','S11A','R14','R20','R10A','S10A','XH50','XH51','R14.DN','PCB_CN')
       THEN
	       IF V_NUMBER_RECORD_TODAY >= V_MAX_WARNING_RECORDS AND V_MAX_WARNING_RECORDS > 0 THEN
				IF P_CHANNEL='CIC' THEN
					V_REQUEST_STATUS:='REJECTED_M'; 
	            	V_ERR_MSG:='Hỏi tin vượt số lần cho phép!';
				END IF;
				IF P_CHANNEL='PCB' THEN
					raise_application_error( -20000, 'Hỏi tin vượt số lần cho phép!' );
	                RETURN; 
				END IF;
			END IF;
		END IF;
...
--              END IF;
             --DATND15 NCB_CIC_PCB_2023_PM-232 END
              ...
              IF V_COUNT=0 THEN--Tam thoi bo dieu kien check MSGID
...
--              	END IF;
  ...
  END PR_CREATE_CIS_REQUEST;
   ...
   IF V_CHANEL = 'PCB'
   THEN V_STATUS_BY_CHANEL := 'STATUS_REQUEST_PCB';
	   /*DATND15 fixed for jira NCB_CIC_PCB_2023_PM-241 Start*/
	  	IF p_err_code IS NOT NULL THEN
	  		BEGIN
	  		SELECT ITEM_VALUE_VN INTO V_ERR_MSG FROM SYS_BUNDLE sb   
	  		WHERE APPLICATION_ID = 'CIS' 
	  		AND ITEM_CODE = p_err_code 
	  		AND ITEM_GROUP = 'PCB_ERROR';
	  		EXCEPTION WHEN OTHERS THEN
	  			V_ERR_MSG := 'Đã có lỗi xảy ra, yêu cầu liên hệ IT hỗ trợ.';
	  		END;
	  	END IF;
	  	/*DATND15 fixed for jira NCB_CIC_PCB_2023_PM-241 End*/
   ELSE V_STATUS_BY_CHANEL := 'STATUS_REQUEST';
   END IF;

    OPEN p_out FOR
        SELECT CIS_REQUEST.CUSTOMER_NAME, CIS_REQUEST.MSG_ID, CIS_REQUEST.APPLICATION_ID, CIS_REQUEST.REGISTER_NO, CIS_REQUEST.OPERATION_TYPE, CIS_REQUEST.PAY_PERIODICITY,
            CIS_REQUEST.DOC_TYPE, CIS_REQUEST.CUSTOMER_CODE, CIS_REQUEST.CCY, CIS_REQUEST.BORROWER, 
             /*DATND15 fixed for jira NCB_CIC_PCB_2023_PM-241 Start*/
            --CIS_REQUEST.ERR_MSG,
             V_ERR_MSG	ERR_MSG,
            /*DATND15 fixed for jira NCB_CIC_PCB_2023_PM-241 End*/
            CIS_REQUEST.NOTE, CIS_REQUEST.ADDRESS,
            CIS_REQUEST.ID, CIS_REQUEST.MAKER, CIS_REQUEST.TASK_ID, CIS_REQUEST.EMAIL, CIS_REQUEST.PCB_CODE, CIS_REQUEST.COUNTRY_OF_BIRTH,
            CIS_REQUEST.GENDER, CIS_REQUEST.FLAG, CIS_REQUEST.LAST_VERSION, CIS_REQUEST.USERNAME_REQUEST, CIS_REQUEST.CIC_CODE, CIS_REQUEST.STATUS,
            CIS_REQUEST.TAX_CODE, CIS_REQUEST.ID_NO, CIS_REQUEST.PRODUCT_CODE, CIS_REQUEST.MEMBER_CODE, CIS_REQUEST.CIS_NO, CIS_REQUEST.CHANNEL,
            CIS_REQUEST.ERR_CODE, CIS_REQUEST.BRANCH_CODE, CIS_REQUEST.CUSTOMER_TYPE, CIS_REQUEST.REQUESTED_DATE, CIS_REQUEST.RESPONSE_DATE, 
        ...
        -- vinhnc5 custom api end
        left JOIN SYS_REFCODE refStatus ON CIS_REQUEST.STATUS = refStatus.REF_CODE AND refStatus.REF_GROUP = V_STATUS_BY_CHANEL
        left JOIN SYS_REFCODE refCustomerType ON CIS_REQUEST.CUSTOMER_TYPE = refCustomerType.REF_CODE AND refCustomerType.REF_GROUP = 'LOAI_KH'
        LEFT JOIN SYS_REFCODE refDocType ON CIS_REQUEST.DOC_TYPE = refDocType.REF_CODE AND refDocType.REF_GROUP = 'DOC_TYPE'
        LEFT JOIN SYS_REFCODE refProduct ON CIS_REQUEST.PRODUCT_CODE = refProduct.REF_CODE AND refProduct.REF_GROUP = 'LS_PRODUCT'
        WHERE  CIS_REQUEST.CIS_NO = p_cis_no;        
           -- vinhnc5 custom api end
...
IF V_STATUS='WARNING1' OR V_STATUS='WARNING2' OR V_STATUS='WARNING3' OR V_STATUS='WARNING1WARNING3' OR V_STATUS='WARNING2WARNING3' OR V_STATUS='NEWCUST' THEN
...
END IF;
    ...
    SELECT V_STATUS errors
           ...
           ,CASE WHEN V_STATUS='WARNING1' OR V_STATUS='WARNING2'  OR V_STATUS='WARNING3' OR V_STATUS='WARNING1WARNING3' OR V_STATUS='WARNING2WARNING3' or V_STATUS='NEWCUST' THEN V_MSGID ELSE '' END FLAG
           ,V_CIS_NO CIS_NO
           ,V_NOTE NOTE
...
PROCEDURE PR_CHECK_REQUEST_INFO_CIC ( p_id_no CIS_REQUEST.ID_NO%TYPE,
                                      ...
                                      p_product_code CIS_REQUEST.PRODUCT_CODE%TYPE,
                                      p_cic_code varchar2,
                                      p_is_batch varchar2 default('N'),--Datnd15 CB_CIC_PCB_2023_PM-232
                                      p_user_ip	 varchar2 DEFAULT(''),--Datnd15 CB_CIC_PCB_2023_PM-232
                                      p_record_number	 NUMBER DEFAULT(0),--Datnd15 CB_CIC_PCB_2023_PM-232
                                      p_out    OUT SYS_REFCURSOR)
...
IS
   ...
   V_CHECK_RESULT_ACCEPT	VARCHAR2(1);
  --Datnd15 NCB_CIC_PCB_2023_PM-232 END
    --vinhnc5 cap nhat api
   V_REQUESTED_DATE CIS_REQUEST.REQUESTED_DATE%TYPE;
   V_RESPONSE_DATE CIS_REQUEST.RESPONSE_DATE%TYPE;
  --vinhnc5 end
BEGIN
...
pkg_system_log.PR_LOG_EXCEPTION('Datnd15_FIS','DEBUG WARNING',NULL,V_PARAMETER);
COMMIT;
--V_STATUS := 'WARNING1';
IF V_STATUS LIKE 'WAITING%' THEN
...
/*Datnd15 NCB_CIC_PCB_2023_PM-232 Start*/
	IF p_is_batch = 'Y' THEN
		SELECT MAX(PKG_UTILITY.IS_NUMBER(A.PAR1)) INTO V_MAX_WARNING_RECORDS FROM SYS_GROUPS A LEFT JOIN SYS_USER B ON ',' || B.GROUP_NAME || ',' LIKE '%,' || A.GROUP_NAME || ',%' WHERE B.USER_NAME = p_user_ip;
		V_NUMBER_RECORD_TODAY := PCK_CIS_REQUEST.CHECK_LIMIT_ENQUERY(p_user_ip);
		IF p_product_code IN ('R21','R11A','S11A','R14','R20','R10A','S10A','XH50','XH51','R14.DN','PCB_CN') AND V_MAX_WARNING_RECORDS > 0 THEN
			IF V_CIS_NO IS NOT NULL AND V_NUMBER_RECORD_TODAY >= V_MAX_WARNING_RECORDS THEN 
					V_STATUS :='REJECTED_M';
			ELSE
					IF p_record_number > (V_MAX_WARNING_RECORDS - V_NUMBER_RECORD_TODAY) THEN
						V_STATUS :='REJECTED_M';
					END IF;
			END IF;
		END IF;
		IF V_STATUS ='REJECTED_M' THEN
			OPEN p_out FOR
				SELECT V_STATUS errors
					   ,'Hỏi tin vượt số lần cho phép!' errors_mess
					   ,'1' FLAG
					   ,V_STATUS STATUS
					   ,p_user_ip maker
				FROM DUAL;
...
BEGIN
 ...
 IF p_channel='CIC' THEN
 	/*p_param7 -- record number in file excel with: p_param7 - 7 + 1*/
 	pkg_system_log.PR_LOG_EXCEPTION('Datnd15_FIS','DEBUG WARNING',NULL,p_param7);COMMIT;
 	PR_CHECK_REQUEST_INFO_CIC ( p_id_no => p_param1 ,
                              p_register_no => p_param2,
                              p_tax_code => p_param3,
                              p_customer_type => p_param4,
                              p_product_code => p_param5,
                              p_cic_code => p_param6,
                              p_is_batch => 'Y',--Datnd15 Datnd15 NCB_CIC_PCB_2023_PM-232
                              p_user_ip => p_user,--Datnd15 Datnd15 NCB_CIC_PCB_2023_PM-232
                              p_record_number => (TO_NUMBER(p_param7) - 7 +1),--Datnd15 Datnd15 NCB_CIC_PCB_2023_PM-232
                              p_out =>p_out);
...
END;
/*DATND15 DATND15 NCB_CIC_PCB_2023_PM-232 STRAT*/
FUNCTION CHECK_LIMIT_ENQUERY(
			user_name IN VARCHAR2
) RETURN NUMBER
IS
...
BEGIN
	V_TOTAL_RECORD := 0; 
	SELECT COUNT(1) INTO V_TOTAL_RECORD FROM CIS_REQUEST cr WHERE 
	TRUNC(CREATED_DATE) = TRUNC(SYSDATE) 
	AND MAKER = user_name 
	AND PRODUCT_CODE IN ('R21','R11A','S11A','R14','R20','R10A','S10A','XH50','XH51','R14.DN','PCB_CN');	
	RETURN V_TOTAL_RECORD;
END;

FUNCTION CHECK_RECORD_ACCEPT_SEND(p_task_id 	IN VARCHAR2,--LO1250
								  p_cis_no		IN VARCHAR2,--8372
								  max_recored	IN NUMBER,--5
								  record_in_day	IN NUMBER) RETURN varchar2 --4
IS
v_total_record_accept NUMBER DEFAULT(0);
v_count_record		  NUMBER DEFAULT(0);
v_count_record_1	  NUMBER DEFAULT(0);
v_result			  VARCHAR2(1);
BEGIN
	v_result := 'N';
	IF record_in_day < max_recored THEN
		v_count_record := 0;
		v_total_record_accept := max_recored - record_in_day;
		
		SELECT count(1) INTO v_count_record FROM (
			SELECT total.CIS_NO FROM (
				SELECT CIS_NO ,ROW_NUMBER() OVER (ORDER BY CIS_NO ASC) AS rn  FROM CIS_REQUEST cr WHERE 
				cr.TASK_ID  = p_task_id
				AND PRODUCT_CODE IN ('R21','R11A','S11A','R14','R20','R10A','S10A','XH50','XH51','R14.DN','PCB_CN')
				ORDER BY ID ASC ) total 
			WHERE 
				rn <= v_total_record_accept ) table_record WHERE table_record.CIS_NO = p_cis_no;
	...
	RETURN v_result;
END;
/*DATND15 DATND15 NCB_CIC_PCB_2023_PM-232 END*/
/*Coding custom API Start*/
PROCEDURE IS_VALID_USER_EXITS(p_user   IN VARCHAR2,
							  P_OUT    OUT SYS_REFCURSOR) AS
v_count_user	NUMBER;
...
BEGIN
	SELECT count(1) INTO v_count_user FROM SYS_USER t WHERE UPPER(t.USER_NAME) = UPPER(p_user) AND t.STATUS = '1';
	IF v_count_user > 0 THEN
		SELECT t.MEMBER_CODE INTO v_department FROM SYS_USER t WHERE UPPER(t.USER_NAME) = UPPER(p_user) AND t.STATUS = '1';
		SELECT count(1) INTO v_count_user FROM SYS_REFCODE sr WHERE sr.REF_GROUP = 'LS_BRANCH' AND sr.REF_CODE = v_department AND sr.STATUS = '1';
	END IF;
	IF v_count_user > 0 THEN
		OPEN P_OUT FOR
		SELECT 'Y' ERRORS FROM dual;
	ELSE
		OPEN P_OUT FOR
		SELECT 'N' ERRORS FROM dual;
	END IF;
END;
/*Coding custom API END*/
END PCK_CIS_REQUEST;